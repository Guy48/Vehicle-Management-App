...
function createVehicle(id, licensePlate, status = "Available") {
    if (id.length !== 8)
        throw new Error("Invalid id length");
    if (!/^[A-Z0-9]{5,8}$/.test(licensePlate))
...
function updateVehicle(id, updates) {
    ...
    if (updates.id && updates.id !== id) {
        if (vehicles.find(v => v.id === updates.id))
            throw new Error("Duplicate id");
        if (updates.id.length !== 8)
            throw new Error("Invalid id length");
    }
...
describe("Vehicle System — Complex Scenarios and Edge Cases", () => {
    ...
    test("prevents creating more than 2 vehicles in maintenance", () => {
        ...
        expect(() => createVehicle("A3333333", "CAR003", "Maintenance")).toThrow("Too many vehicles in maintenance");
    });
    test("cannot delete a car that is in use or maintenance", () => {
        ...
        expect(() => deleteVehicle("U1111111")).toThrow("Cannot delete vehicle that is in use or under maintenance");
        expect(() => deleteVehicle("M1111111")).toThrow("Cannot delete vehicle that is in use or under maintenance");
    });
    test("edit licensePlate to an invalid one fails", () => {
        createVehicle("V1111111", "AAA111");
        expect(() => updateVehicle("V1111111", { licensePlate: "bad!!" })).toThrow("Invalid license plate");
    });
    test("rename vehicle id to one that already exists fails", () => {
        createVehicle("X1111111", "CAR111");
        createVehicle("X2222222", "CAR222");
        expect(() => updateVehicle("X1111111", { id: "X2222222" })).toThrow("Duplicate id");
    });
    test("perform sequence: create → update → delete → reuse", () => {
        ...
        expect(() => deleteVehicle("SEQ11111")).toThrow("Cannot delete vehicle that is in use or under maintenance");
    ...
    test("simulate messy operations and ensure consistency", () => {
        ...
        expect(() => updateVehicle("BBBBBBB2", { licensePlate: "AAA11" })).toThrow("Duplicate license plate");
        updateVehicle("AAAAAAA1", { status: "Maintenance" });
        createVehicle("CCCCCCC3", "CCC33", "Maintenance");
        expect(() => createVehicle("DDDDDDD4", "DDD44", "Maintenance")).toThrow("Too many vehicles in maintenance");
    ...
    test("system handles empty state gracefully", () => {
        ...
        expect(() => updateVehicle("MISSING1", { status: "Available" })).toThrow("Vehicle not found");
    });
});
