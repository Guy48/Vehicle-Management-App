// backend/src/controllers/vehicle_controller.ts
...
import {
  ...
  isValidLicense,
  genId,
} from "../services/vehicle_service.js";
...
export function createVehicle(req: Request, res: Response) {
  ...
  const rawLicense =
    typeof req.body?.licensePlate === "string" ? req.body.licensePlate.trim() : req.body?.licensePlate;
  ...
  if (!isValidLicense(licensePlate)) {
    return res.status(400).json({
      error:
        "Invalid licensePlate format. Use only uppercase letters, digits and hyphen (e.g. ABC-123 or ABC123).",
    });
  }
...
export function resetFromSeedHandler(req: Request, res: Response) {
  try {
    resetFromSeed();
    return res.status(204).send();
  } catch (err) {
